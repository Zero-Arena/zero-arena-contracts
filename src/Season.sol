...
contract Season is Ownable2Step, ReentrancyGuard {
    ...
    // Permissionless after endTime. `sortedTokens` is the off-chain ranking;
    // we verify it's monotonically decreasing in liveTotalReturnBps. Entries
    // beyond index 2 are ignored for payout (display-only).
